...
def k_medoid(D, k):
    ...
    medoids = []
    for i in range(0, k):
        medoids_posi = random.randint(0, m)
    ...
    while cal_num <= 10:
        ...
        for i in range(0, m):
            distance = []
            for j in range(0, k):
                distance.append(np.linalg.norm(medoids[j] - D[i], 1))
            k_cluster = np.argmin(distance)
            # return where the D(i) belongs
...
def new_medoid(k_cluster_vector):
    ...
    for i in range(0, len(k_cluster_vector)):
        ...
        for j in range(0, len(k_cluster_vector)):
            distance += np.linalg.norm(k_cluster_vector[i] - k_cluster_vector[j], 1)
